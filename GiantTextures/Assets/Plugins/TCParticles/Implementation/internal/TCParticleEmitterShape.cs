...
namespace TC.Internal {
	...
	public class ParticleEmitterShape {
		...
		static ComputeBuffer s_positionBuffer;
		...
		public void SetListData(ComputeShader cs, int kern, string name) {

			if (s_positionBuffer == null || s_positionBuffer.count < positions.Length) {
				if (s_positionBuffer != null) {
					s_positionBuffer.Release();
				}

				//Vector3 stride
				s_positionBuffer = new ComputeBuffer(positions.Length, 12);
			}
			s_positionBuffer.SetData(positions);
			cs.SetBuffer(kern, name, s_positionBuffer);
		}
		...
		public void ReleaseData() {
			...
			if (s_positionBuffer != null && positions != null) {
				s_positionBuffer.Release();
			}
